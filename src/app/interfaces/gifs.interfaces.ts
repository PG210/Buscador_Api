// To parse this data:
//
//   import { Convert, SearchGifs } from "./file";
//
//   const searchGifs = Convert.toSearchGifs(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SearchGifs {
    data:       Gif[]; //hacemos el cambio a Gif
    pagination: Pagination;
    meta:       Meta;
}

export interface Gif {//hacemo el cambio a Gif
    type:                       Type;
    id:                         string;
    url:                        string;
    slug:                       string;
...
export class Convert {
    public static toSearchGifs(json: string): SearchGifs {
        return cast(JSON.parse(json), r("SearchGifs"));
    }

    public static searchGifsToJson(value: SearchGifs): string {
        return JSON.stringify(uncast(value, r("SearchGifs")), null, 2);
    }
}
